...
final class RequiredFields {
	...
	static BoundingBox readBoundingBox(ReadBuffer readBuffer) {
	...
	static void readMagicByte(ReadBuffer readBuffer) throws IOException {
		...
		if (!readBuffer.readFromFile(magicByteLength + 4)) {
			throw new IOException("reading magic byte has failed");
		}
		...
		if (!BINARY_OSM_MAGIC_BYTE.equals(magicByte)) {
			throw new IOException("invalid magic byte: " + magicByte);
		}
	}

	static long readMapDate(ReadBuffer readBuffer) throws IOException {
		...
		if (mapDate < 1200000000000L) {
			throw new IOException("invalid map date: " + mapDate);
		}
		return mapDate;
	}

	static Tag[] readPoiTags(ReadBuffer readBuffer) throws IOException {
		// get and check the number of POI tags (2 bytes)
		int numberOfPoiTags = readBuffer.readShort();
		if (numberOfPoiTags < 0) {
			throw new IOException("invalid number of POI tags: " + numberOfPoiTags);
		}
		...
		for (int currentTagId = 0; currentTagId < numberOfPoiTags; ++currentTagId) {
			...
			if (tag == null) {
				throw new IOException("POI tag must not be null: " + currentTagId);
			}
	...
	static String readProjectionName(ReadBuffer readBuffer) throws IOException {
	...
	static void readRemainingHeader(ReadBuffer readBuffer) throws IOException {
		...
		if (remainingHeaderSize < HEADER_SIZE_MIN || remainingHeaderSize > HEADER_SIZE_MAX) {
			throw new IOException("invalid remaining header size: " + remainingHeaderSize);
		}

		// read the header data into the buffer
		if (!readBuffer.readFromFile(remainingHeaderSize)) {
			throw new IOException("reading header data has failed: " + remainingHeaderSize);
		}
	}

	static int readTilePixelSize(ReadBuffer readBuffer) throws IOException {
		// get and check the tile pixel size (2 bytes)
		int tilePixelSize = readBuffer.readShort();
		if (tilePixelSize != Tile.TILE_SIZE) {
			throw new IOException("unsupported tile pixel size: " + tilePixelSize);
		}
		return tilePixelSize;
	}

	static Tag[] readWayTags(ReadBuffer readBuffer) throws IOException {
		// get and check the number of way tags (2 bytes)
		int numberOfWayTags = readBuffer.readShort();
		if (numberOfWayTags < 0) {
			throw new IOException("invalid number of way tags: " + numberOfWayTags);
		}
		...
		for (int currentTagId = 0; currentTagId < numberOfWayTags; ++currentTagId) {
			...
			String tag = readBuffer.readUTF8EncodedString();
			if (tag == null) {
				throw new IOException("way tag must not be null: " + currentTagId);
			}
